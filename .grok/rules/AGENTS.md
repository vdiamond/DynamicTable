...
| `TableExtention/` | Extension methods (`UtilExtensions`) for CSV loading, joins, sorting, `DataTable` interop, and POCO mapping |
...
**Note:** The folder is spelled `TableExtention` (not `Extension`). Preserve this spelling in paths and references.
...
- Target **net8.0** for new and updated projects unless a specific subproject requires otherwise.
...
- **`TableExtention`** adds extension methods on `IDynamicTable`, `IColumn`, `IColumns`, and `IRow`/`IRows` — keep new helpers here rather than bloating the core library unless they are fundamental primitives.
...
- Do not rename `TableExtention` or move projects without updating all `ProjectReference` paths.

## Working with Grok

- Before implementing features, read the relevant interfaces in `Table/Table/ITable.cs` and the concrete types they bind to.
- When adding table operations, check whether an extension already exists in `TableExtention/Util.cs` before creating duplicates.
...
- Do not "fix" the `TableExtention` folder spelling unless explicitly asked.
